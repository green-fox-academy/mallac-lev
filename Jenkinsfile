pipeline {
  ...
  stages {
    ...
    stage('Deploy to EB') {
      ...
      steps{
        withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', accessKeyVariable: 'AWS_ACCESS_KEY_ID', credentialsId: 'malachitedevops', secretKeyVariable: 'AWS_SECRET_ACCESS_KEY']]) {
